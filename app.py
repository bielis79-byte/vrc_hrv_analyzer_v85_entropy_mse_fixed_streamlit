...
st.set_page_config(page_title="VRC / HRV RRi Analyzer Pro v8.5", layout="wide")
...
def hvg_wide_table(long_df):
    """
    Tabla ancha HVG comparativa incluyendo tipo de grafo.
    """
...
def enforce_entropy_consistency(metrics, rr_entropy):
    ...
    """
    try:
        ent = sample_entropy_common(rr_entropy, m=2, r_factor=0.2, r_reference=rr_entropy)
        mse_vals = mse_common(rr_entropy, scales=20, m=2, r_factor=0.2, r_reference=rr_entropy)
        metrics["SampEn"] = ent
        metrics.update(mse_vals)
    except Exception:
        pass
...
st.title("VRC / HRV RRi Analyzer Pro v8.5")
